...
def main(args):
    ...
    if MODE == 'train':
        ...
        CT_path = TRAIN_DIR
        MRI_path = LABEL_DIR
        full_dataset = CTDataset(CT_image_root=CT_path, MRI_label_root=MRI_path)
        # Split into train and test sets
        train_set_size = int(full_dataset.__len__() * 0.8)
        ...
        valid_set_size = len(train_set) - train_set_size1
        train_set, valid_set = data.random_split(train_set, [train_set_size1, valid_set_size])
        print(f"Train data set: {len(train_set)}")
        print(f"Valid data set: {len(valid_set)}")
        print(f"Test data set: {len(test_set)}")
        Batch_Size = BATCH_SIZE  # Assuming BATCH_SIZE is predefined
